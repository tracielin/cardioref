...
if __name__ == "__main__":
    ...
    use_case.add_node("early_hsPDA", label="Consider early targeted hsPDA prophylaxis with indomethacin (<24 hours) \nin selected infants by predefined criteria (e.g., male sex, ELBW, GA<26 weeks, low unit spontaneous closure rate)")
    use_case.add_node("done_after_hsPDA", label="done!")
    use_case.add_edge("early_hsPDA", "done_after_hsPDA", label="effective", color="green")
    ...
    use_case.add_node("pda_ib", label="consider early targeted treatment of infants with PDA with ibuprofen\n or indomethacin (<6 days postnatally and >2 L/minute, >0.25 FiO2)")
    use_case.add_edge("ecg_assess", "pda_ib")
    use_case.add_node("done_after_pda", label="done!")
    use_case.add_edge("pda_ib", "done_after_pda", color="green")
    ...
    use_case.add_node("hsPDA_ib", label="consider treatment of symptomatic (> 2L/minute, >0.25 FiO2)\n infants with hsPDA with ibuprofen (>=6 days postnatally)")
    ...
    use_case.add_node("close_duct", label="consider definite ductal closure by catheter intervention\n or surgical ligation if persistent and symptomatic over time")
    use_case.add_edge( "consider rescue treatment (paracetamol)", "close_duct", label="if not effective", color="red")
